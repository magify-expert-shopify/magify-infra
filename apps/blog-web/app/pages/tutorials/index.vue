...
const {
  ...
  status,
  refresh,
} = await useAsyncData("tutorials:pages", () => listPages(), {
  default: () => [],
});
...
const filteredPages = computed(() =>
  (pages.value ?? [])
    .filter((page) => page.pageType === "TUTORIAL")
    .sort((left, right) =>
  ...
  <section class="space-y-6">
    ...
    <header class="space-y-1">
      ...
      <h1 class="text-2xl font-semibold text-slate-900">
        Tutoriels du projet courant
      </h1>
      <p class="text-sm text-slate-500">
        Liste des pages de type tutoriel rattachées au projet courant.
      </p>
    </header>
    ...
    <FeedbackRichMessage
      ...
      title="Impossible de charger les tutoriels"
      description="Les pages de type tutoriel n’ont pas pu être récupérées."
    ...
    <p v-else class="text-sm text-slate-500">
      Aucune page de type tutoriel n'est rattachée à ce projet pour le moment.
    </p>
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1">
          ...
          <p class="text-sm text-sky-800/80">
            Consulte la page des suggestions de tutoriels pour repérer
            rapidement les opportunités à rédiger.
          </p>
        </div>

        <UButton
          to="/suggestions/tutorials"
          color="primary"
          variant="solid"
          icon="i-lucide-lightbulb"
        >
          Voir les suggestions de tutoriels
        </UButton>
      </div>
    </div>
  </section>
