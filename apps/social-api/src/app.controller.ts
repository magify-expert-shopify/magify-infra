...
export class AppController {
  ...
  private async checkRedis() {
    ...
    const host = process.env.BULLMQ_HOST?.trim() || process.env.REDIS_HOST?.trim() || '127.0.0.1';
    const port = Number(process.env.BULLMQ_PORT ?? process.env.REDIS_PORT ?? 6379);
    const password = process.env.BULLMQ_PASSWORD?.trim() || process.env.REDIS_PASSWORD?.trim();
    ...
    try {
      ...
      return {
        ...
        ok: pong === 'PONG',
        detail:
          pong === 'PONG' ? 'Connected to Redis' : `Unexpected reply: ${pong}`,
      };
    } catch (error) {
      return {
        label: 'Redis',
...
<html lang="fr">
  <head>
    ...
    <title>Social Magify API Status</title>
    <style>
      :root {
        color-scheme: dark;
        --bg: #07111f;
