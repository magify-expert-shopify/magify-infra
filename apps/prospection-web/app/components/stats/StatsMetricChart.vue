...
import type { StatsMetricSeries } from "~/types/stats";
...
const averageValue = computed(() => {
  if (!props.series.points.length) {
    return 0;
  }

  const total = props.series.points.reduce(
    (sum, point) => sum + Number(point.value || 0),
    0,
  );
  return total / props.series.points.length;
});
...
function getBarClass(point: StatsMetricSeries["points"][number]) {
  if (point.isBusinessDay === false) {
    return "bg-slate-300";
  }

  return "bg-gradient-to-t from-primary-700 via-primary-500 to-primary-300";
}
</script>
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div
          class="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700"
        >
          {{ formatAverage(averageValue) }} moyenne
        </div>
      </div>
    </div>
